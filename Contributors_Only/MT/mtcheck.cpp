...
//to access the RNG initialise using a constructor with a seed and access the member function using that object

int main()
{
    auto currentTime = chrono::high_resolution_clock::now().time_since_epoch().count();//generating the seed
    uint32_t seed = currentTime;
    MT randnum(seed);//initialising a constructor to MT class
    vector<int>freq(10,0);
    std::ofstream outputFile("chisqdata.txt", std::ios::trunc);

    for (int i = 0; i < 1000; i++)
    {

      outputFile<<randnum.getran()<<" ";
      if(i%100==99)
      outputFile<<endl;
    }
